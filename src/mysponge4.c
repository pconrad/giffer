...
int main(int argc, char **argv)
{
    ...
    fprintf(stderr,"About to call makeColorMap\n");

    ColorMapObject *palette = makeColorMap(hexColors, numColors);

    GifPixelType paletteToNewColor[numColors];

    fprintf(stderr,"About to call GifUnionColorMap\n");
    ColorMapObject *merged = GifUnionColorMap(GifFileIn->SColorMap,
					      palette,
					      paletteToNewColor);

    if (merged==NULL) {
      fprintf(stderr,"Error in file %s line %d: could not merge color maps\n",
	      __FILE__,__LINE__);
      exit(2);
    }

    int rectangleColor = paletteToNewColor[0];
    ...
    if (GifFileIn->ImageCount > 0) {
      ...
      GifDrawRectangle(image, left + w/4, top+h/4, w/8, h/8, rectangleColor);
    } else {
    ...
    fprintf(stderr,"merged->ColorCount=%d\n",merged->ColorCount);
    fflush(stderr);
